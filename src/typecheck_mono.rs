//! 型推論 + wasm 向けの型情報抽出
//!
//! `typecheck.rs` をコピーして拡張したもの。比較学習用に独立ファイルとして残してある。
//!
//! 追加点:
//! - `Type` enum: resolve 済みの型を外向きに表現
//! - `resolve_to_type`: TypeId を Type に展開する
//! - `FunctionScheme`: top-level 関数の型情報
//! - `type_check_with_info`: type_check + 関数情報の抽出
...
/// resolve 済みの型表現。typecheck の外向きの型。
///
/// `TypeId` のような内部識別子を含まず、後段（monomorphization, wasm 生成）が直接扱える。
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub enum Type {
    /// 具象型 ("Number", "String", "Boolean", "Void", "Array")
    Concrete(String),
    /// 未解決の型変数。id は元の db のインデックスを流用（同じ Var は同じ id）。
    Var(u32),
    /// 関数型
    Function {
        params: Vec<Type>,
...
impl TypeChecker {
    ...
    /// TypeId を resolve 済みの `Type` に変換する。
    ///
    /// `Var` は同一性を保つために id をそのまま流用する
    /// （同じ TypeId の Var は同じ `Type::Var(id)` になる）。
    fn resolve_to_type(&mut self, id: TypeId) -> Type {
        let id = self.resolve(id);
        match self.db[id].clone() {
            DbEntry::Var => Type::Var(id as u32),
            DbEntry::Concrete(name) => Type::Concrete(name),
            DbEntry::Function {
                params,
                return_type,
            } => Type::Function {
                params: params.iter().map(|p| self.resolve_to_type(*p)).collect(),
                return_type: Box::new(self.resolve_to_type(return_type)),
            },
            DbEntry::Symlink(_) => unreachable!("resolve should collapse Symlinks"),
        }
    }
...
// =============================================================================
// wasm 向けの追加 API: 関数の型情報を resolve した形で取り出す
// =============================================================================

/// top-level の関数定義の型情報。
///
/// monomorphization の specialization 元となる。`var_ids` が非空なら多相。
#[derive(Debug, Clone)]
pub struct FunctionScheme {
    /// 量化された型変数の id (`Type::Var(id)` と同じ id)
    pub var_ids: Vec<u32>,
    /// パラメータの型 (量化変数は `Type::Var(id)` として現れる)
...
/// `type_check_with_info` の結果。
#[derive(Debug)]
pub struct TypeCheckResult {
    pub errors: Vec<TypeError>,
    /// 関数名 → スキーム (top-level の `const xxx = (...) => {...}` だけ抽出)
    pub functions: HashMap<String, FunctionScheme>,
}

/// 型推論を実行し、エラーと top-level 関数の型情報を返す。
///
/// 既存の `type_check` は変更せず、wasm 後段に必要な情報抽出を追加で行う。
pub fn type_check_with_info(statements: &[Statement]) -> TypeCheckResult {
    let mut checker = TypeChecker::new();
    for stmt in statements {
        checker.visit_statement(stmt);
    }

    // top-level の各 const arrow function について、解決済みの型情報を集める
    let mut functions = HashMap::new();
    for stmt in statements {
        let Statement::ConstDeclaration { name, init, .. } = stmt else {
            continue;
        };
        if !matches!(init, Expression::ArrowFunction { .. }) {
            continue;
        }
        let Some(scheme) = checker.scope.get(name).cloned() else {
            continue;
        };
        let var_ids: Vec<u32> = scheme.vars.iter().map(|&id| id as u32).collect();
        let resolved = checker.resolve_to_type(scheme.body);
        let Type::Function {
            params,
            return_type,
        } = resolved
...
mod tests {
    ...
    #[test]
    fn info_extracts_monomorphic_function() {
        let stmts = crate::compile("const inc = (x) => { return x + 1; };");
        let result = type_check_with_info(&stmts);
        let inc = &result.functions["inc"];
        assert!(!inc.is_polymorphic());
        assert_eq!(inc.param_types, vec![Type::Concrete("Number".into())]);
        assert_eq!(inc.return_type, Type::Concrete("Number".into()));
    }

    #[test]
    fn info_extracts_polymorphic_function() {
        let stmts = crate::compile("const id = (x) => { return x; };");
        let result = type_check_with_info(&stmts);
        let id = &result.functions["id"];
        assert!(id.is_polymorphic());
        assert_eq!(id.var_ids.len(), 1);
        // (T) → T なので、引数と戻り値が同じ Var id
        let Type::Var(p) = &id.param_types[0] else {
            panic!("expected Var");
        };
        let Type::Var(r) = &id.return_type else {
            panic!("expected Var");
        };
        assert_eq!(p, r, "both should be the same type variable id");
    }

    #[test]
    fn info_skips_non_function_const() {
        let stmts = crate::compile("const x = 42;");
        let result = type_check_with_info(&stmts);
        assert!(!result.functions.contains_key("x"));
    }

    #[test]
    fn info_collects_multiple_functions() {
        let stmts = crate::compile(
            "const id = (x) => { return x; };
             const inc = (x) => { return x + 1; };",
        );
        let result = type_check_with_info(&stmts);
        assert!(result.functions["id"].is_polymorphic());
        assert!(!result.functions["inc"].is_polymorphic());
    }
}
